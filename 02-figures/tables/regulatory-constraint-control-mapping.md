...
| Hybrid reconciliation ambiguity (off-chain vs on-chain truth gaps) | Inconsistent registers, audit failures, disputes | Event-sourced off-chain system of record (append-only) + deterministic evidence regeneration + reconciliations | event log exports, projection rebuild reports, reconciliation reports |
